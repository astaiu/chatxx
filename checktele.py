...
@astathon.on(events.NewMessage(outgoing=True, pattern=r"\.تثبيت (.*)"))
async def _(event):
    if ispay2[0] == "yes":
        trys = 0
        ...
        if msg[0] == "تلقائي":  # تثبيت تلقائي عدد يوزر قناة
            ...
            @astathon.on(events.NewMessage(outgoing=True, pattern=r"\.حالة التثبيت التلقائي"))
            async def _(event):
                if "on" in isauto:
...
┗━━━━━┛
    ...
    {str(eee)}''')
            ...
            isclaim.clear()
            isclaim.append("off")
            await astathon.send_message(event.chat_id, "انتهى التثبيت")
